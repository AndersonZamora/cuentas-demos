...
import { IoFilmOutline, IoPencilSharp, IoRemoveCircleOutline, IoSearch } from 'react-icons/io5';
...
export const TableSupplier = ({ suppliers }: Props) => {
    ...
    const handleDelete = (id: string) => {
        Swal.fire({
            title: "!Atención!",
            icon: "warning",
            text: 'Estas seguro de eliminar la el proveedor?',
            ...
            if (result.isConfirmed) {
                ...
                loadingAlert('Eliminando proveedor...');

                const { status, message } = await deleteSupplier(id);
                            ...
                            filteredSuppliers.map(supp => (
                                <tr key={supp.id}>
                                    <th className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-2xl whitespace-nowrap p-4 text-left text-blueGray-700 ">
                                        <h3>{capitalize(supp.name)}</h3>
                                    </th>
                                    <th className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-2xl whitespace-nowrap p-4 text-left text-blueGray-700 ">
                                        <h3>{capitalize(supp.company)}</h3>
                                    </th>
                                    <th className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-2xl whitespace-nowrap p-4 text-left text-blueGray-700 ">
                                        <h3>{capitalize(supp.contact)}</h3>
                                    </th>
                                    <th className="flex flex-row gap-2 border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4 text-left text-blueGray-700 ">
                                        <Link
