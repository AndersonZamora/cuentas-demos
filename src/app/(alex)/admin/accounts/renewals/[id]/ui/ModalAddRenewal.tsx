...
import { createNewRenewals } from '@/actions';
import { HeaderModal } from '@/components';
import { ICategory, ISupplier } from '@/interfaces';
...
export const ModalAddRenewal = ({ idAccount, categorys, suppliers }: Props) => {

    const router = useRouter();
    const { register, handleSubmit, setValue, watch, setError, reset, formState: { errors } } = useForm<RAccount>();

    const [openModalAddRenewal, setOpenModalAddRenewal] = useState(false);

    const handleOnSubmit = async (data: RAccount) => {
        
        loadingAlert('Renovando...');

        const { status, message } = await createNewRenewals({
            ...data,
            id: idAccount
        });
    ...
    return (
        <>
            <div className="flex flex-col items-center mt-2">
                <button
                    onClick={() => setOpenModalAddRenewal(true)}
            ...
            </div>
            {
                ...
                (
                    <HeaderModal
                        ...
                        openModal={openModalAddRenewal}
                        handleClose={() => setOpenModalAddRenewal(false)}
                    >
                        <form className='fade-in px-2 md:px-4' onSubmit={handleSubmit(handleOnSubmit)} noValidate autoComplete="off">
